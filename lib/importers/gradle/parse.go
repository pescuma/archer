...
func parseDeps(projects *model.Projects, content string, projsInsideRoot map[string]bool) error {
	...
	for _, l := range lines {
		if state == waitingRoot {
			...
			if rootMatches != nil {
				p := projects.GetOrCreate(rootMatches[1])
		...
		if state == parsingDeps {
			...
			p := projects.GetOrCreate(depName)
